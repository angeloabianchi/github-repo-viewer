...
const DisplayRepoInfo = ({ repoSelected }) => {
    ...
    useEffect(() => {
        ...
        setTimeout( async () => {
            try {
                const getData = await (fetchData('languages', repoSelected['owner'].login, repoSelected['name']));
                        ...
                        <div class='row'>
                            <div class="progress-stacked">
                                {Object.keys(languages).map((key) => (
                                    <div class="progress" role="progressbar" aria-label="Success example" aria-valuenow={languages[key]} aria-valuemin="0" aria-valuemax="10000">
                                        <div class="progress-bar overflow-visible text-dark" style={{"width": "25%"}}>{key}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
